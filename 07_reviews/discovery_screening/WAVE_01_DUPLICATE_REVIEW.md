...
The raw compiler created title-only possible-duplicate groups without collapsing them. There are 40 groups containing 81 PMID records. Primary readers compared available bibliographic metadata before using `X_DUPLICATE`; the fresh third-repair independent audit then received every group member plus the other members' title, year, journal, authors, PMID, DOI, and source URL. The second-repair counts previously recorded here are historical and superseded.

The authoritative third-repair Wave 1 primary ledger assigns `X_DUPLICATE` to 64 rows overall, including 55 of the 81 possible-title-group rows. The fresh audit assigns `X_DUPLICATE` to one possible-title-group row. Within the 81-row possible-title set, 22 complete primary/audit triples agree and 59 remain open conflicts; none ends with an agreed final `X_DUPLICATE` label. The full Wave 1 final ledger contains three `X_DUPLICATE` rows outside these title-only groups. No key was collapsed or deleted.

Identifier-only cross-wave reconciliation is a separate layer. All 192 exact Wave 1/Wave 2 key overlaps remain deterministic Wave 2 `exclude / blank type / X_DUPLICATE` primaries retaining the identical key. Forty-eight were independently audited and all 48 agreed; the remaining 144 were outside the formula selection. Thus all 192 remain final Wave 2 `X_DUPLICATE` exclusions, while the global index still preserves them as Wave 2 provenance overlaps.
